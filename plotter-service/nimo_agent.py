...
def poll_next_job(api_base: str, plotter_id: str, interval_s: int = 15):
    while True:
        try:
            r = requests.get(f"{api_base}/jobs/next",
                             params={"plotter_id": plotter_id}, timeout=30)
            ...
            # mark started
            requests.post(f"{api_base}/jobs/{job_id}/status",
                          json={"status": "started", "notes": "processing"}, timeout=30)
            ...
            # mark completed
            requests.post(f"{api_base}/jobs/{job_id}/status",
                          json={"status": "completed", "notes": "done"}, timeout=30)
...
def main():
    ...
    p.add_argument("--api", required=True,
                   help="API base URL, e.g. https://nimo.fly.dev")
    p.add_argument("--plotter-id", default="NIMO-01",
                   help="Plotter identifier")
