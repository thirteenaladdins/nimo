<script>
  ...
  async function fetchDailyArt() {
    ...
    // Only fetch if we don't have today's art cached
    if (dailyArtCache && lastFetchDate === today) {
      dailyArt = dailyArtCache;
      return;
    }
...
<main class="p-4">
  ...
  <div class="mb-8">
    <h2 class="text-xl font-semibold mb-2">Image generated by Nimo</h2>
    <input
      type="text"
      bind:value={imageUrl}
